...
% ============================================================================
% Aetherium Automata -- Appendices (English)
% ============================================================================

\chapter{Repository Overview}
\label{app:repo}

This appendix lists the main repository components relevant to the thesis.

\begin{description}
  \item[Engine (C++)] \texttt{src/engine/} contains the CLI, YAML parser, and validator.
  \item[Transports (C++)] \texttt{src/transport/} contains transport interfaces and stubs.
  \item[Gateway (Elixir/Phoenix)] \texttt{src/gateway/aetherium\_gateway/} contains the Phoenix application and channels.
  \item[Server (Elixir)] \texttt{src/server/aetherium\_server/} contains the gateway-connecting agent.
  \item[IDE (Electron/React)] \texttt{src/ide/} contains the Electron application.
  \item[Docs] \texttt{docs/} contains the YAML specification, architecture notes, and protocol drafts.
  \item[Tests] \texttt{tests/} contains Python-based CLI tests and a CLI behavior spec.
\end{description}

\chapter{Build and Run Instructions}
\label{app:run}

The following commands reflect the repository structure at the time of writing.
They are provided as guidance; exact dependencies (compilers, Elixir/Node versions) depend on the local environment.
...
./build/aetherium_engine --version
./build/aetherium_engine --validate example/automata/automata-yaml-examples/one-state-automata-inline.yaml
\end{verbatim}

\section{Gateway (Elixir/Phoenix)}
...
\chapter{Notes on Current Limitations}
\label{app:limits}

\begin{itemize}
  \item The IDE is implemented as an Electron desktop application, not as a browser-hosted web application.
  \item Engine validation may terminate the process on malformed YAML due to assertion-style checks.
  \item Transport implementations (MQTT/UDP/DDE) and hardware access (GPIO/I2C) are not implemented.
  \item Execution semantics for timed automata and time-travel debugging are currently not implemented in the runtime; the IDE includes mock UI for these features.
\end{itemize}
...
The style of literature in the template is from Ing. Radek Pyšný \cite{Pysny}, whose work was improved by prof. Adam Herout, dr. Jaroslav Dytrych, dr. Petr Veigend and Mr. Karel Hanák to comply with the norm and support all frequently used types of citations. Its documentation can be found in the appendix
...
\begin{description}
	...
	\item[\href{http://texstudio.sourceforge.net/}{TeXstudio}] Portable open-source GUI for \LaTeX{}. Ctrl+click switches between source text and PDF. Integrated spell checker\footnote{Spell checker for Czech version can be installed from \url{https://extensions.openoffice.org/de/project/czech-dictionary-pack-ceske-slovniky-cs-cz}}, syntax highlighter etc. To use this tool, you need to first install MikTeX or another \LaTeX{} distribution.
	...
	\item[\href{https://inkscape.org/en/download/}{InkScape}] Portable open-source vector graphic (SVG and PDF) editor. Excellent tool to use to create images for technical text. Difficult to master, but the results are worth it.
...
\chapter{Writing texts in English}
\label{anglicky}
...
Articles in English are a nightmare and almost all of us fail to use them correctly. The basic rule is, that if there's a particular noun, it's preceded by ``the''. Definite articles must be in following phrases:
\begin{verbatim}
...
\begin{samepage}
\noindent The use of ``a'' and ``an'' is based on the pronunciation, rather than how the word is written:
\begin{verbatim}
  an HMM
  an XML
  a universal model
...
\begin{itemize}
  ...
  \item{Comma is a powerful tool and you should use it for your sentence structure. Use a~comma to separate the initial dependent clause from the main independent clause. Sometimes it is appropriate to put a comma just before ``and'' (unlike other languages)!}
\end{itemize}
\begin{verbatim}
...
\paragraph{Common errors in words}
...
\begin{checklist}
    ...
    \item Texts in all images have a font size similar to the surrounding text (neither significantly larger, nor significantly smaller).
...
\begin{checklist}
	...
	\item URL addresses referencing services, projects, sources, GitHub, etc. are referenced using \verb|\footnote{\url{…}}|.
    \item URL addresses in citations are only present, if necessary – article is cited like an article (author, title, where and when was it published), not using URL.
...
\begin{checklist}
    ...
    \item Someone who knows English really well checked the abstract written in Czech or Slovak.
    \item No part of the text is written in the second person (you).
    \item If the first person is used (i, we), a subjective matter is being described (i decided, i~designed, i focused on, i found out, etc.).
    \item There are no colloquialisms in the text.
    \item There are no {\it default} words in the text.
\end{checklist}

\subsection*{Electronic submission, software}
\begin{checklist}
    ...
    \begin{itemize}
    	\item source code (e.g. Matlab, C/C++, Python, \ldots)
    ...
    \begin{itemize}
        ...
        \item \ldots
    \end{itemize}
    \item Source codes are refactored, commented and labeled with an authorship header so that others can tell what they actually are.
...
\begin{checklist}
	\item Do I want to delay (by at most 5 years) the publication ? If so, I will submit an application (in IS) at least a month prior to the submission of the academic work, and I'll include attitude of the company that the intellectual property belongs to and needs to be protected.
    \item I have at least minimum number of standard pages (can be calculated using Makefile and by adding number of pages that images translate to or by using \url{https://app.fit.vut.cz/theses-checker}). If I'm just under the minimum, I consulted my supervisor about it.
    ...
    \item Technical report is bound in a bookbindery.
    ...
    \item The print is signed.
    \item If I'm delaying the publishing, the print contains a data medium with my login written on it using a CD marker (CD marker can be borrowed in library, at Student affairs or when I'm submitting the work).
\end{checklist}
...
\begin{itemize}
  ...
  \item \verb|pdflscape, rotating| -- rotate a page by 90 degrees (for image or table),
...
If you need to reference an equation from the text, you can use command \verb|\eqref|. For example, to reference the equations above \eqref{rovnice2}. If you want to align the equation number vertically, you can use the \texttt{aligned} environment:
...
\chapter{Thesis title is too long}
This chapter was provided by Ing. Zaklová. If your thesis title is too long and does not look good on the first page, you can solve it using solutions provided below.

\section{1st solution}
You can modify the parameters that set the width of various title texts. To do this, uncomment and change the following in the  \texttt{projekt.tex} file:
\begin{itemize}
	\item title.length - for the main title
	\item sectitle.length - for the second title
	\item dectitle.length - for the heading above the declaration (on the page before the Table of Contents)  
\end{itemize}

You have to try different settings for width and pick the correct one. This solution can cause differently sized spaces that can look bad.

\section{2nd solution}
By inserting the newline (\verb|\\|) directly in the title. \\

This change breaks citation and heading above the declaration. To fix these problems, the following changes have to made in the \texttt{fitthesis.cls} file:
\begin{itemize}
	\item replace \verb|\it\@title\rm.| with \verb|\it{\let \\ \empty \@title}\rm.|
	\item block
	...
	\end{verbatim}
\end{itemize}
Do not use this solution with secondary title, it would create an ugly space.
\\ \noindent You can combine both solutions or discuss the problem with your supervisor.

\chapter{Examples of bibliographic citations}
\label{priloha-priklady-citaci}
The czplain style is based on the style created by Mr. Pyšný \cite{Pysny}. This appendix contains a set of supported type of citations with specific examples of bibliographic citations.

Examples are listed on \url{https://www.fit.vut.cz/study/theses/citations/.en#priklady}.

%The next pages of the appendix contain examples of bibliographic citations of the following publications and their parts:
%\begin{itemize}
%   \item Article in a periodical literature (magazine) (pg. \pageref{pr-casopis-clanek}),
%   \item monographic publication (pg. \pageref{pr-monografie}),
%   \item conference proceedings (pg. \pageref{pr-sbornik}),
%   \item conference proceedings entry or book chapter (pg. \pageref{pr-kapitola}),
%   \item manual, documentation, technical report and unpublished materials (pg. \pageref{pr-manual}),
%   \item academic work (pg. \pageref{pr-thesis}),
%   \item web page (pg. \pageref{pr-webpage}),
%   \item and web site (pg. \pageref{pr-website}).
%\end{itemize}
%
