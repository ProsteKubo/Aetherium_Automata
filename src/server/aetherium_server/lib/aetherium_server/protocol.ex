defmodule AetheriumServer.Protocol do
  @moduledoc """
  Binary protocol implementation for Aetherium device communication.
  Mirrors the gateway Protocol module for server-side message handling.
  """
  ...
  @doc "Create hello_ack message"
  @spec hello_ack(String.t()) :: {:ok, binary()}
  def hello_ack(session_id) do
    encode(:hello_ack, %{
  ...
  @doc "Create start command"
  @spec start_command() :: {:ok, binary()}
  def start_command, do: encode(:start, %{})

  @doc "Create stop command"
  @spec stop_command() :: {:ok, binary()}
  def stop_command, do: encode(:stop, %{})

  @doc "Create reset command"
  @spec reset_command() :: {:ok, binary()}
  def reset_command, do: encode(:reset, %{})

  @doc "Create set input message"
  @spec set_input(String.t(), any()) :: {:ok, binary()}
  def set_input(name, value) do
    encode(:set_input, %{name: name, value: value})
  end

  @doc "Create trigger event message"
  @spec trigger_event(String.t(), any()) :: {:ok, binary()}
  def trigger_event(event, data) do
    encode(:trigger_event, %{event: event, data: data})
  end
  ...
  defp encode_payload(:start, _), do: {:ok, <<>>}
  defp encode_payload(:stop, _), do: {:ok, <<>>}
  defp encode_payload(:reset, _), do: {:ok, <<>>}
  ...
  defp encode_payload(:sync_time, _) do
    ts = System.system_time(:millisecond)
    {:ok, <<ts::64-big>>}
  end

  defp encode_payload(:request_state, _), do: {:ok, <<>>}
  defp encode_payload(_type, _payload), do: {:ok, <<>>}
  ...
  defp decode_payload(:state_change, <<ts::64-big, rest::binary>>) do
    with {:ok, from, rest1} <- decode_string(rest),
      ...
      {:ok, %{timestamp: ts, from_state: from, to_state: to, transition_id: tid}}
    end
  end
  ...
  defp decode_payload(:transition_fired, <<ts::64-big, rest::binary>>) do
    with {:ok, from, rest1} <- decode_string(rest),
      ...
      {:ok, %{timestamp: ts, from: from, to: to, transition_id: tid, weight_used: weight}}
    end
  end

  defp decode_payload(:log, <<ts::64-big, level::8, rest::binary>>) do
    with {:ok, message, _rest} <- decode_string(rest) do
      {:ok, %{timestamp: ts, level: byte_to_log_level(level), message: message}}
    end
  end
  ...
  defp decode_payload(:load_ack, _), do: {:ok, %{}}
  defp decode_payload(_type, _payload), do: {:ok, %{}}
  ...
  defp encode_variable_spec(spec) do
    ...
    <<id_bin::binary, name_bin::binary, type_bin::binary, direction_byte::8, default_bin::binary>>
  end
