...
\begin{DESCRIPTION}
  ...
  \item [obrazky-figures] Adresář pro Vaše obrázky.
...
\begin{checklist}
\item Chci práci (na max. 5 let) utajit? Pokud ano, nejpozději měsíc před termínem odevzdání práce si podám žádost (v IS), ke které přiložím případné stanovisko firmy, jejíž duševní vlastnictví je třeba chránit.
\item Mám splněný minimální počet normostran textu (lze spočítat pomocí Makefile a~odhadem přičíst obrázky, případně pomocí \url{https://app.fit.vut.cz/theses-checker}). Pokud jsem těsně pod minimem, konzultoval(a) jsem to s~vedoucím.
...
\item Technickou zprávu mám v deskách z knihařství.
...
\item Prohlášení ve vytištěné verzi práce je podepsané.
\item Při utajeném zveřejnění je ve vytištěné verzi práce paměťové médium. Na médiu je fixkou uveden login (fixku na CD lze zapůjčit v knihovně, na Studijním oddělení nebo až při odevzdání).
\end{checklist}
...
Pokud je třeba rovnici citovat v textu, lze použít příkaz \verb|\eqref|. Například na rovnici výše lze odkázat~\eqref{eq:rovnice2}. Pokud chcete srovnat číslo rovnic u soustavy, lze použít prostředí \texttt{aligned}:
...
Klasický rastrový obrázek se vkládá tak, jak je vidět na obrázku \ref{keepCalm}.
...
Příklady bibliografických citací jsou uvedeny na \url{https://www.fit.vut.cz/study/theses/citations/#priklady}.
